...
def get_metadata(filename):
    """Returns (default_min, default_max, expected_mass, particle_name) for a filename."""
    fname = filename.split('/')[-1].split('?')[0]
    if fname in KNOWN_METADATA:
        m = KNOWN_METADATA[fname]
        return m["min"], m["max"], m["mass"], m["name"]
    default_particle = fname.replace('.root', '').replace('.csv', '').split('_')[0]
    return 0.0, 120.0, 0.0, default_particle


def build_file_options(available_files):
